    ...
    <div class="py-12 bg-gradient-to-b from-gray-50 to-gray-100 min-h-screen">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-10">
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl p-8 sm:p-10 lg:p-12 border border-gray-200">
                <h3 class="text-3xl font-extrabold text-green-700 mb-8 text-center">{{ __('កែប្រែព័ត៌មានអ្នកប្រើប្រាស់') . ': ' . ($user->name ?? 'N/A') }}</h3>
                ...
                <form method="POST" action="{{ route('admin.update-user', $user->id) }}" enctype="multipart/form-data"
                    ...
                    <div class="space-y-6">
                        <div class="mb-6">
                            ...
                            <x-text-input id="name" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-green-600 focus:ring-green-600 py-3 px-4 transition duration-150 ease-in-out" type="text" name="name" :value="old('name', $user->name)" required autofocus autocomplete="name" />
                        ...
                        <div class="mb-6">
                            ...
                            <select id="role" name="role" x-model="userRole" class="block w-full rounded-xl border-gray-300 focus:border-green-600 focus:ring-green-600 rounded-md shadow-sm py-3 px-4 transition duration-150 ease-in-out" required>
                                <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>{{ __('Admin') }}</option>
                                <option value="professor" {{ old('role', $user->role) == 'professor' ? 'selected' : '' }}>{{ __('Professor') }}</option>
                    ...
                    <div class="border-t border-gray-200/50 pt-8 mt-8">
                             ...
                             <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <x-text-input id="email" class="block w-full rounded-xl py-3 px-4" type="email" name="email" :value="old('email', $user->email)" required autocomplete="username" />
                                        <x-input-error :messages="$errors->get('email')" class="mt-2" />
                                    </div>
                                    <div>
                                        <x-input-label for="password" class="flex items-center text-lg text-gray-700 font-semibold mb-2"><i class="fas fa-lock mr-3 text-green-500"></i> {{ __('ពាក្យសម្ងាត់ថ្មី (ទុកឱ្យនៅទទេប្រសិនបើមិនចង់ផ្លាស់ប្តូរ)') }}</x-input-label>
                                        <x-text-input id="password" class="block w-full rounded-xl py-3 px-4" type="password" name="password" autocomplete="new-password" />
                                        <x-input-error :messages="$errors->get('password')" class="mt-2" />
                                    </div>
                                    <div class="col-span-1 md:col-span-2">
                                        <x-input-label for="password_confirmation" class="flex items-center text-lg text-gray-700 font-semibold mb-2"><i class="fas fa-check-circle mr-3 text-green-500"></i> {{ __('បញ្ជាក់ពាក្យសម្ងាត់ថ្មី') }}</x-input-label>
                                        <x-text-input id="password_confirmation" class="block w-full rounded-xl py-3 px-4" type="password" name="password_confirmation" autocomplete="new-password" />
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <x-text-input id="student_id_code" class="block w-full rounded-xl py-3 px-4" type="text" name="student_id_code" :value="old('student_id_code', $user->student_id_code)" required />
                        ...
                        <div>
                            ...
                            <select name="generation" id="generation" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select id="department_id" name="department_id" class="block w-full rounded-xl py-3 px-4" required>
                                            <option value="">{{ __('សូមជ្រើសរើសមហាវិទ្យាល័យជាមុនសិន') }}</option>
                                        </select>
