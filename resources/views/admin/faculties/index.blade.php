<x-app-layout>
    <div class="py-12 bg-gray-100 min-h-screen font-sans">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div x-data="{ viewMode: 'grid' }" {{-- Alpine state for toggling view --}}
                ...
                <div class="mt-8">
                    @if ($faculties->isEmpty())
                        <div class="col-span-1 md:col-span-2 lg:col-span-3 bg-white p-12 rounded-3xl text-center text-gray-400 shadow-xl border border-gray-100">
                            ...
                            <p class="font-semibold text-lg">{{ __('មិនទាន់មានមហាវិទ្យាល័យណាមួយនៅឡើយទេ។') }}</p>
                                ...
                                @foreach ($faculties as $faculty)
                                    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 flex flex-col justify-between hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                                        ...
                                        <div class="space-y-2 mb-6">
                                            <p class="text-gray-700 font-medium"><span class="font-bold text-gray-800">{{ __('ប្រធានមហាវិទ្យាល័យ') }}</span>: <span class="text-gray-600">{{ $faculty->dean->name ?? 'N/A' }}</span></p>
                                        </div>
                                        <div class="flex justify-end space-x-3 mt-auto">

                                            {{-- Edit button --}}
                                            <a href="{{ route('admin.edit-faculty', $faculty->id) }}" class="p-3 bg-gray-100 rounded-full text-green-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('កែប្រែ') }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                             ...
                                             <button type="button" onclick="openDeleteModal('{{$faculty->id }}')" class="p-3 bg-gray-100 rounded-full text-red-600 hover:bg-gray-200 transition duration-150 ease-in-out" title="{{ __('លុប') }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider hidden sm:table-cell">
                                                {{ __('ឈ្មោះមហាវិទ្យាល័យ (English)') }}
                                            </th>
                                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider hidden lg:table-cell">
                                                {{ __('ប្រធាន') }}
                                            </th>
                                        ...
                                        @foreach ($faculties as $faculty)
                                            <tr class="hover:bg-gray-50 transition duration-150">
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 w-1/12">
                                                    {{ $i++ + (($faculties->currentPage() - 1) * $faculties->perPage()) }}
                                                </td>
...
<script>
    ...
    function openDeleteModal(facultyId ) {
        ...
        deleteForm.action = `/admin/faculties/${facultyId}`;
