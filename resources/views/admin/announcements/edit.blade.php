            ...
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl p-8 lg:p-12 border border-gray-200 transition-all duration-300 transform hover:shadow-3xl">
                ...
                <div class="mb-8 pb-4 border-b border-gray-200">
                    <h2 class="font-extrabold text-4xl text-gray-900 leading-tight">
                        {{ __('កែសម្រួលសេចក្តីប្រកាស') }}
                    </h2>
                    <p class="mt-2 text-lg text-gray-500">{{ __('កែប្រែព័ត៌មានលម្អិតនៃសេចក្តីប្រកាសនេះ') }}</p>
                </div>
                    ...
                    <div class="space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input type="text" name="title_km" id="title_km" value="{{ old('title_km', $announcement->title_km) }}" required class="form-input w-full rounded-xl border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500 transition duration-150 ease-in-out placeholder-gray-400" placeholder="{{ __('បញ្ចូលចំណងជើងជាភាសាខ្មែរ') }}">
                            </div>
                            <div>
                                <label for="title_en" class="block text-sm font-semibold text-gray-700 mb-1">{{ __('ចំណងជើង (អង់គ្លេស)') }}</label>
                                <input type="text" name="title_en" id="title_en" value="{{ old('title_en', $announcement->title_en) }}" required class="form-input w-full rounded-xl border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500 transition duration-150 ease-in-out placeholder-gray-400" placeholder="{{ __('បញ្ចូលចំណងជើងជាភាសាអង់គ្លេស') }}">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <textarea name="content_km" id="content_km" rows="6" required class="form-textarea w-full rounded-xl border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500 transition duration-150 ease-in-out placeholder-gray-400" placeholder="{{ __('បញ្ចូលខ្លឹមសារជាភាសាខ្មែរ') }}">{{ old('content_km', $announcement->content_km) }}</textarea>
                            </div>
                            <div>
                                <label for="content_en" class="block text-sm font-semibold text-gray-700 mb-1">{{ __('ខ្លឹមសារ (អង់គ្លេស)') }}</label>
                                <textarea name="content_en" id="content_en" rows="6" required class="form-textarea w-full rounded-xl border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500 transition duration-150 ease-in-out placeholder-gray-400" placeholder="{{ __('បញ្ចូលខ្លឹមសារជាភាសាអង់គ្លេស') }}">{{ old('content_en', $announcement->content_en) }}</textarea>
                            </div>
                        </div>
                            ...
                            {{-- <div>
                                ...
                                <select name="course_offering_id" id="course_offering_id" class="form-select w-full rounded-xl border-gray-300 shadow-sm focus:ring-green-500 focus:border-green-500 transition duration-150 ease-in-out">
                                    ...
                                    @foreach($courseOfferings as $offering)
                                        <option value="{{ $offering->id }}" {{ old('course_offering_id', $announcement->course_offering_id) == $offering->id ? 'selected' : '' }}>
                                            {{ $offering->course->name_en }} ({{ $offering->program->name_en }})
                                        </option>
                                    @endforeach
                                </select>
                            </div> --}}
                    ...
                    <div class="mt-12 flex justify-between items-center">
                        ...
                        <button type="submit" class="px-8 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white font-bold rounded-full shadow-lg hover:from-green-600 hover:to-green-700 transition duration-300 transform hover:scale-105 flex items-center space-x-2">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M7.707 9.293a1 1 0 010-1.414L10.586 5H7a1 1 0 110-2h5.586a1 1 0 01.707.293l3.414 3.414a1 1 0 010 1.414l-3.414 3.414A1 1 0 0113 12.586V10h-2v3a1 1 0 11-2 0V9z" />
                                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
                            </svg>
                        </button>
                    </div>
