<x-guest-layout>
    <style>
        ...
        body {
            ...
            margin: 0; padding: 0; overflow-x: hidden; background: #020617;
        }
        ...
        .portal-wrapper::before {
            ...
            background-size: 45px 45px; pointer-events: none; z-index: 1;
        }
        ...
        .glass-portal-card {
            background: rgba(15, 23, 42, 0.65); backdrop-filter: blur(25px) saturate(180%);
            -webkit-backdrop-filter: blur(25px) saturate(180%); border: 1px solid rgba(16, 185, 129, 0.2);
            box-shadow: 0 40px 100px -20px rgba(0, 0, 0, 0.8);
        }

        .focus-green:focus {
            border-color: var(--primary-green) !important; box-shadow: 0 0 0 4px rgba(16, 185, 129, 0.2);
    ...
    <div class="portal-wrapper" x-data="{ loginMode: 'email' }">
        ...
        <div class="w-full sm:max-w-[460px] glass-portal-card p-10 sm:p-12 overflow-hidden sm:rounded-[3rem] reveal" style="animation-delay: 0.1s">
            ...
            <div class="flex p-1 bg-white/5 rounded-2xl mb-10 border border-white/5">
                <button @click="loginMode = 'email'" 
                    :class="loginMode === 'email' ? 'active' : 'text-gray-500 hover:text-gray-300'"
                    class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest tab-btn border border-transparent">
                    <i class="fa-solid fa-envelope text-sm"></i> Email
                </button>
                <button @click="loginMode = 'qr'" 
                    :class="loginMode === 'qr' ? 'active' : 'text-gray-500 hover:text-gray-300'"
                    class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest tab-btn border border-transparent">
            ...
            <div x-show="loginMode === 'email'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-7">
                    ...
                    <button type="submit" class="w-full rounded-2xl bg-emerald-600 px-8 py-5 font-black text-white transition-all hover:bg-emerald-500 shadow-2xl shadow-emerald-900/40 uppercase tracking-[0.2em]">
                        {{ __('ចូលប្រើប្រព័ន្ធ') }}
                    </button>
                </form>
            </div>
            ...
            <div x-show="loginMode === 'qr'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-x-4">
                <div class="text-center">
                    ...
                    <div class="inline-block p-4 bg-white rounded-[2rem] shadow-[0_0_50px_rgba(16,185,129,0.2)] border-4 border-emerald-500/30">
                        {{-- បង្ហាញ QR Code (បងត្រូវប្រាកដថាផ្ញើ $qrCode ពី Controller មក) --}}
                        @if(isset($qrCode))
                            {!! $qrCode !!}
                        @else
    ...
    <script>
        ...
        channel.bind('login-success', function(data) {
            document.getElementById('qr-status').innerText = "ជោគជ័យ! កំពុងចូលប្រព័ន្ធ...";
            window.location.href = "/qr-login/finalize/{{ $token }}";
        });
    </script>
    @endif
